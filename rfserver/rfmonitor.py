...
class RFMonitor(Ipc.IpcMessageProcessor):
	...
	self.ipc = MongoIpc.MongoIpc(RFMONITOR_ID, RFMONITOR_RFPROXY_CHANNEL)
        #self.ipc = MongoIpc.MongoIPCMessageService(MONGO_ADDRESS,
        #                                          MONGO_DB_NAME,
        #                                         RFMONITOR_ID,
        #                                        threading.Thread,
        #                                       time.sleep)
        self.ipc.parallel_listen(self)
	...
	message.set_from(RFMONITOR_ID)
        message.set_to(str(0))
